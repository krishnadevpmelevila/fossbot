...
// Welcome Message 
bot.on('guildMemberAdd', (member) => {
  member.roles.add('984841687522238504');

});
bot.on('messageCreate', (message) => {
  console.log("ok");
  if (message.author.id != '907633615599452160' && message.author.id != '282859044593598464' && message.author.id != '270904126974590976' && message.author.id != '707101869964656721' && message.content.toLowerCase() != 'clearchat' && message.channel.id != '984852138599534752'&&message.channel.id != '984816997420396607') {
    console.log("ok");
      ...
      axios.get(process.env.API_URL + '/data', {
        ...
        if (!hasMatch) {
          if (message.member != null) {
            message.member.roles.add('984841687522238504');
            message.member.roles.remove('984841332013031494');
...
bot.on('messageCreate', (message) => {
  ...
  if (message.channelId == '984852138599534752' && message.author.id != '984838832656633856' && message.author.id != '907633615599452160' && message.author.id != '270904126974590976' && message.content.toLowerCase() != 'clearchat') {
        ...
        if (hasMatch) {
          axios.get(process.env.API_URL + '/activate/' + user._id, {
            ...
            axios.get(process.env.API_URL + '/add/' + user._id + '/' + message.author.id, {
              ...
              message.reply(usersname + ', You have been verified!, IMPORTANT: Dont forget to enter this code on quizess platfrom while it is asked! All the best!')
              //  add role
              console.log(message.content);
              message.member.roles.add('984841332013031494');
              message.member.roles.remove('984841687522238504');
